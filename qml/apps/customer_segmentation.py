...
clusters=pd.read_csv(DATA_SUMMARY_PATH.joinpath("elbow_cluster_df.csv"))
...
def plot_no_of_clusters(clusters):
    ...
    fig = go.Figure()
    fig.add_trace(go.Scatter(x=clusters.key, y=clusters.value,
...
# rfm=train_model(rfm_scaled,rfm)



def kmeans_customer_segments(rfm):
    rfm_category_df=rfm.groupby(["Kmeans Cluster Category"], as_index=False )["CustomerID"].count()
    rfm_category_df=rfm_category_df.sort_values(by=['CustomerID'],ascending=False)
    rfm_category_df.columns=['Clusters','No. Customers']
    colors=['gold','orange','grey','brown']
    fig=px.bar(rfm_category_df,x='Clusters',y='No. Customers',text='No. Customers',color='Clusters',
...
def plot_clusters(rfm):
    ...
    fig = px.scatter(rfm, x="Recency", y="Frequency", color="Kmeans Cluster Category",color_discrete_sequence=colors,
                     hover_data=['Kmeans Cluster'], log_y=True,title="Customer Clusters")
    fig.update_layout(legend=dict(yanchor="top",y=0.99,xanchor="left",x=0.8),autosize=True,margin=dict(t=30,b=0,l=0,r=0))
    return fig
    

def plot_3d_clusters(rfm):
    colors=['orange','grey','gold','brown']
    fig = px.scatter_3d(rfm, x="Recency", y="Frequency", z='Kmeans Cluster',color_discrete_sequence=colors,
        ...
        dbc.Row(
            ...
            dbc.Col(html.Div([                  
                    dcc.Graph(
                            ...
                            figure=kmeans_customer_segments(rfm),
                            config={'displayModeBar': False }
                            ),
                          ] 
                          ),
...
## CODE FOR ONLINE PROCESING AND MODEL TRAINING
